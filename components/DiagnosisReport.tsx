              ...
              <div className="space-y-3">
                {diagnosis.lifestyle_triggers.map((trigger, i) => (
                  <div key={i} className="bg-white p-3 rounded-lg shadow-sm border border-indigo-50">
                    <p className="text-xs font-bold text-indigo-400 uppercase mb-1">{trigger.issue}</p>
                    <p className="text-sm font-semibold text-slate-800 mb-1">
                      Did you know? <span className="text-indigo-700">{trigger.trigger}</span> could be the cause.
                    </p>
                    <p className="text-xs text-slate-500 italic">Try this: {trigger.habit}</p>
                  </div>
                ))}
              </div>
            </div>
                ...
                <div className="bg-slate-50 p-6 rounded-lg border border-slate-100">
                    ...
                    {diagnosis.lifestyle_triggers && diagnosis.lifestyle_triggers.length > 0 && (
                      <div className="pt-4 border-t border-slate-200">
                        <p className="text-xs font-bold text-slate-400 uppercase mb-2">Lifestyle Insights</p>
                        <div className="space-y-2">
                           {diagnosis.lifestyle_triggers.slice(0, 2).map((trigger, i) => (
                             <div key={i} className="text-sm text-indigo-900">
                               <span className="font-bold text-indigo-700">{trigger.issue}:</span> {trigger.trigger}
                             </div>
                           ))}
                        </div>
                      </div>
